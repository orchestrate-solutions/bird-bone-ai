...
class CICDPipeline:
    ...
    def add_result(self, stage: PipelineStage, success: bool, message: str, 
    ...
    def run_validation_stage(self) -> bool:
        ...
        try:
            ...
            validation_ctx = setup_environment_validation.setup_environment_validation(self.context.copy())
            ...
            self.add_result(
                ...
                success,
                "Environment validation completed" if success else "Environment validation failed",
                {
    ...
    def run_security_stage(self) -> bool:
        ...
        try:
            ...
            security_ctx = configure_security_settings.configure_security_settings(self.context.copy())
            ...
            critical_failures = [check for check in security_checks 
            ...
            self.add_result(
                ...
                success,
                "Security checks completed" if success else f"Security issues found: {len(critical_failures)} critical",
                {
    ...
    def run_testing_stage(self) -> bool:
        ...
        try:
            ...
            testing_ctx = setup_testing_pipeline.setup_testing_pipeline(self.context.copy())
            ...
            critical_test_failures = [result for result in test_results 
            ...
            self.add_result(
                ...
                success,
                "Testing stage completed" if success else f"Critical test failures: {len(critical_test_failures)}",
                {
    ...
    def run_deployment_stage(self) -> bool:
        ...
        try:
            ...
            deployment_failures = [check for check in deployment_checks if not check['passed']]
            ...
            self.add_result(
                ...
                success,
                "Deployment readiness confirmed" if success else f"Deployment issues: {len(deployment_failures)}",
                {
                    'middleware_results': middleware_ctx,
    ...
    def run_full_pipeline(self, stages: Optional[List[PipelineStage]] = None) -> Dict[str, Any]:
            ...
            if stage in stage_methods:
                ...
                if not stage_success:
                    ...
                    if stage in [PipelineStage.VALIDATION, PipelineStage.TESTING]:
                        logger.warning(f"Critical stage {stage.value} failed, but continuing pipeline")
    ...
    def generate_pipeline_report(self, overall_success: bool, total_duration: float) -> Dict[str, Any]:
        ...
        for result in self.results:
            ...
            print(f"{status_icon} {result.stage.value.upper()}: {result.message} ({result.duration:.2f}s)")

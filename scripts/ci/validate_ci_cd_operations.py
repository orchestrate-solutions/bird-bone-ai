...
from middleware import security_middleware, validation_middleware, testing_middleware, deployment_middleware
...
class CICDValidator:
    ...
    def add_result(self, component: str, check: str, status: ValidationStatus, 
        ...
        # Log the result
        level = logging.INFO if status == ValidationStatus.PASSED else \
                logging.WARNING if status == ValidationStatus.WARNING else \
                logging.ERROR
    ...
    def validate_project_structure(self) -> None:
        ...
        for file_path in required_files:
            ...
            if full_path.exists():
                self.add_result(
                    "project_structure", f"file_{file_path.replace('/', '_')}",
                    ValidationStatus.PASSED, f"Found {file_path}"
                )
            else:
                self.add_result(
                    "project_structure", f"file_{file_path.replace('/', '_')}",
                    ValidationStatus.WARNING, f"Missing {file_path}"
                )
        ...
        for script in required_scripts:
            ...
            if script_path.exists():
                try:
                    ...
                    self.add_result(
                        "ci_cd_scripts", script.replace('.py', ''),
                        ValidationStatus.PASSED, f"Script {script} is valid"
                    )
                except SyntaxError as e:
                    self.add_result(
                        "ci_cd_scripts", script.replace('.py', ''),
                        ValidationStatus.FAILED, f"Syntax error in {script}: {e}"
                    )
            else:
                self.add_result(
                    "ci_cd_scripts", script.replace('.py', ''),
                    ValidationStatus.FAILED, f"Missing script: {script}"
                )
        ...
        for name, middleware in middlewares:
            try:
                ...
                if hasattr(middleware, 'process'):
                    self.add_result(
                        "middleware", name,
                        ValidationStatus.PASSED, f"{name} middleware initialized successfully"
                    )
                else:
                    self.add_result(
                        "middleware", name,
                        ValidationStatus.WARNING, f"{name} middleware missing process method"
                    )
            except Exception as e:
                self.add_result(
                    "middleware", name,
                    ValidationStatus.FAILED, f"{name} middleware failed: {e}"
                )
    ...
    def validate_environment_setup(self) -> None:
        ...
        try:
            ctx = {}
            validated_ctx = setup_environment_validation.setup_environment_validation(ctx)
            
            if validated_ctx.get('environment_validated'):
                self.add_result(
                    "environment", "validation",
                    ValidationStatus.PASSED, "Environment validation passed",
                    validated_ctx
                )
            else:
                self.add_result(
                    "environment", "validation",
                    ValidationStatus.WARNING, "Environment validation had issues",
                    validated_ctx
                )
        except Exception as e:
            self.add_result(
                "environment", "validation",
                ValidationStatus.FAILED, f"Environment validation failed: {e}"
            )
        ...
        try:
            ...
            if secured_ctx.get('security_configured'):
                self.add_result(
                    ...
                    ValidationStatus.PASSED, "Security configuration completed",
                    secured_ctx
                )
            else:
                self.add_result(
                    "security", "configuration",
                    ValidationStatus.WARNING, "Security configuration incomplete",
                    secured_ctx
                )
        except Exception as e:
            self.add_result(
                "security", "configuration",
                ValidationStatus.FAILED, f"Security configuration failed: {e}"
            )
    ...
    def validate_testing_pipeline(self) -> None:
        ...
        try:
            ...
            if tested_ctx.get('testing_pipeline_setup'):
                self.add_result(
                    "testing", "pipeline_setup",
                    ValidationStatus.PASSED, "Testing pipeline setup completed",
                    tested_ctx
                )
            else:
                self.add_result(
                    "testing", "pipeline_setup",
                    ValidationStatus.WARNING, "Testing pipeline setup incomplete",
                    tested_ctx
                )
        except Exception as e:
            self.add_result(
                "testing", "pipeline_setup",
                ValidationStatus.FAILED, f"Testing pipeline setup failed: {e}"
            )
    ...
    def validate_github_workflows(self) -> None:
        ...
        if not workflows_dir.exists():
            self.add_result(
                ...
                ValidationStatus.WARNING, "No .github/workflows directory found"
            )
            return
        ...
        if not workflow_files:
            self.add_result(
                "github", "workflow_files",
                ValidationStatus.WARNING, "No GitHub workflow files found"
            )
            return
        ...
        for workflow_file in workflow_files:
            try:
                ...
                if 'on' in workflow_data and 'jobs' in workflow_data:
                    self.add_result(
                        "github", f"workflow_{workflow_file.stem}",
                        ValidationStatus.PASSED, f"Workflow {workflow_file.name} is valid"
                    )
                else:
                    self.add_result(
                        "github", f"workflow_{workflow_file.stem}",
                        ValidationStatus.WARNING, f"Workflow {workflow_file.name} missing required sections"
                    )
            except Exception as e:
                self.add_result(
                    "github", f"workflow_{workflow_file.stem}",
                    ValidationStatus.FAILED, f"Invalid workflow {workflow_file.name}: {e}"
                )
            ...
            try:
                ...
                if requirements:
                    self.add_result(
                        ...
                        ValidationStatus.PASSED, "requirements.txt exists and has content"
                    )
                else:
                    self.add_result(
                        "dependencies", "requirements_txt",
                        ValidationStatus.WARNING, "requirements.txt is empty"
                    )
            except Exception as e:
                self.add_result(
                    "dependencies", "requirements_txt",
                    ValidationStatus.FAILED, f"Error reading requirements.txt: {e}"
                )
        else:
            self.add_result(
                "dependencies", "requirements_txt",
                ValidationStatus.WARNING, "No requirements.txt found"
            )
            ...
            try:
                ...
                if 'project' in pyproject_data or 'tool' in pyproject_data:
                    self.add_result(
                        ...
                        ValidationStatus.PASSED, "pyproject.toml is valid"
                    )
                else:
                    self.add_result(
                        "dependencies", "pyproject_toml",
                        ValidationStatus.WARNING, "pyproject.toml exists but may be incomplete"
                    )
            except ImportError:
                self.add_result(
                    "dependencies", "pyproject_toml",
                    ValidationStatus.WARNING, "Cannot validate pyproject.toml (tomli not installed)"
                )
            except Exception as e:
                self.add_result(
                    "dependencies", "pyproject_toml",
                    ValidationStatus.FAILED, f"Error reading pyproject.toml: {e}"
                )
        else:
            self.add_result(
                "dependencies", "pyproject_toml",
                ValidationStatus.WARNING, "No pyproject.toml found"
            )
    ...
    def run_comprehensive_validation(self) -> Dict[str, Any]:
        ...
        for check in validation_checks:
            try:
                ...
                self.add_result(
                    "validation", check.__name__,
                    ValidationStatus.FAILED, f"Check failed: {e}"
                )
...
def validate_ci_cd_operations(ctx: Ctx) -> Ctx:
    ...
    try:
        ...
        ctx['ci_cd_operations_validated'] = report['overall_status'] in ['passed', 'warning']
...
__all__ = ['validate_ci_cd_operations', 'CICDValidator', 'ValidationStatus', 'ValidationResult']

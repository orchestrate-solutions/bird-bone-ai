...
def ci_error_handler_middleware(ctx: Ctx, next_func) -> Ctx:
    ...
    try:
        ...
        error_info = {
            ...
            'timestamp': ctx.get('timestamp', 'unknown') # Assuming timestamp might be in context
        }
        
        return {
            **ctx, 
            'errors': ctx.get('errors', []) + [error_info]
        }
        ...
        error_info = {
            ...
            'operation': ctx.get('operation', 'unknown_ci_operation'),
            'details': str(e),
            'filename': str(e.filename) if hasattr(e, 'filename') and e.filename else None,

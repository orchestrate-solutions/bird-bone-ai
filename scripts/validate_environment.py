...
class EnvironmentValidator:
    ...
    def validate_required_packages(self) -> Dict[str, bool]:
        ...
        print_status(f"Package validation: {successful_imports}/{len(required_packages)} ({success_rate:.1f}%)")
    ...
    def validate_cuda_and_gpu(self) -> Dict[str, any]:
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                print_success(f"CUDA {torch.version.cuda} available with {gpu_info['gpu_count']} GPU(s)")
                ...
                if not gpu_info['optimal_setup']:
                    warning_msg = "Consider using H100/A100 GPUs for optimal performance"
    ...
    def validate_dvc_configuration(self) -> bool:
        ...
        try:
            ...
            if Path('.dvc').exists():
                ...
                # Check DVC config
                try:
                    result = subprocess.run(['dvc', 'config', '--list'], 
                                          capture_output=True, text=True, check=True)
                    if self.verbose:
    ...
    def validate_precommit_hooks(self) -> bool:
        ...
        try:
            ...
            if Path('.pre-commit-config.yaml').exists():
                try:
                    result = subprocess.run(['pre-commit', 'hook-impl', '--help'], 
                                          capture_output=True, text=True, check=True)
                    print_success("Pre-commit hooks configuration found ✓")
                    return True
                except subprocess.CalledProcessError:
                    warning_msg = "Pre-commit hooks not installed. Run: pre-commit install"
                    print_warning(warning_msg)
                    self.warnings.append(warning_msg)
                    return False
            else:
    ...
    def generate_health_report(self) -> Dict[str, any]:
        ...
        if self.results['package_validation']:
            package_success_rate = sum(self.results['package_validation'].values()) / len(self.results['package_validation'])
            total_checks += 1
        ...
        print_colored(f"Overall Health: {health_status} ({health_score:.1f}%)", health_color)
